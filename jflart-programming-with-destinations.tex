...
The other operations on difference lists follow easily, as we can observe on table~\ref{table:impl-dlist}:
\begin{table}[th]
\small
...
toList :: DList a ⊸ [a]
toList i = unur . fromRegion $ i <&> \dl → dl & fill @'[]
\end{minted}
...
\begin{itemize}
  ...
  \item \mintinline{haskell}`concat` calls \mintinline{haskell}`fillComp`, which fills the destination of the first difference list with the root of the second one. The resulting \mintinline{haskell}`Incomplete` object hence has the same root as the first list, holds the elements of both lists, and has the hole of the second list at the end. Memory-wise, \mintinline{haskell}`concat` will just write the address of the root of the second list into the hole of the first one ; no move is required.
...
mapAccumBFS :: ∀ a b s. (s → a → (s, b)) → s → Tree a → (s, Tree b)
mapAccumBFS f s0 tree =
  allocInRegionAndExtract $
...
\end{minted}
\caption{Implementation of BFS tree traversal with destinations}
\label{table:impl-bfs-tree-traversal}
\end{table}

The signature of \mintinline{haskell}`mapStateBFS` doesn't involve linear types. They are only put into use in the implementation of the said function, as a way to ensure memory safety when using destinations, as one can observe in table~\ref{table:impl-bfs-tree-traversal}.
...
As a result, nodes of the tree stored in the queue must be wrapped in \mintinline{haskell}`Ur`, to indicate that the queue can still be linearly consumed even though the tree nodes are consumed in a non-linear fashion. On the other hand, the destinations cannot be wrapped in \mintinline{haskell}`Ur`, and as a result their linear consumption is necessary for the queue to be considered as linearly consumed.

The first line of the function is just implementation noise to obtain a destination of a tree \mintinline{haskell}`dtree`. Then the \mintinline{haskell}`go` helper function is called on the initial state with a single-element queue containing the root of tree, and the allocated destination for the resulting tree.
...
The first step mostly takes place in \mintinline{text}`rts/Compact.cmm`, the main C-- module of the RTS for compact regions management, as presented in table~\ref{table:impl-compactAddShallow}.

\begin{table}[th]
\small
\begin{minted}[frame=single,framesep=10pt]{c}
// compactAddShallow# :: Compact# → Addr# → State# RealWorld → (# State# RealWorld, a #)
...
\end{minted}
\caption{Implementation of \texttt{compactAddShallow\#} in the RTS}
\label{table:impl-compactAddShallow}
\end{table}

The \mintinline{c}`stg_compactAddShallowzh` function is mostly a glorified call to the \mintinline{haskell}`ALLOCATE` macro defined in the same file, which tries to do a pointer-bumping alloc in the current block of the compact region if there is enough space, and otherwise add a new block to the region.
  ...
  ReifyStgInfoPtrOp → \_ →  -- we don't care about the function argument (# #)
    opIntoRegsTy $ \[res] resTy → emitAssign (CmmLocal res) $ case resTy of
      ...
      TyConApp _addrLikeTyCon [_typeParamKind, LitTy (StrTyLit sym)] →
        CmmLit (CmmLabel (mkCmmInfoLabel rtsUnitId (fsLit "stg\_" `appendFS` sym)))
...
- How to evaluate naive code (why we use both copyIntoRegion and force)
