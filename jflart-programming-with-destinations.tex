...
We see in part \textbf{A} of Figure~\ref{fig:bench-charts} that the destination-backed difference lists have a comparable memory use as the two other linear implementations, while being quite slower (by a factor 2-4) on all datasets. We expect better results though for the DPS implementation in a language where lazy closures would not be as optimized as in Haskell.
...
In part \textbf{C} of Figure~\ref{fig:bench-charts}, we compare the naive implementation of the S-expression parser and the DPS one (see~\ref{ssec:parser-sexpr}). For this particular program, where using compact regions might reduce the future GC load of the application, it is relevant to benchmark the naive version twice: once with \mintinline{haskellc}`force` and once with \mintinline{haskellc}`compact`.
...
We can also observe that copying the result of the naive version in a compact region (which might be desirable for future GC savings, as detailed in Subsection~\ref{ssec:parser-sexpr}) incurs a real time and memory penalty, that the DPS version offers to avoid.
