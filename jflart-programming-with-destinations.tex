...
Only the implementation of \mintinline{haskellc}`parseSList` will be presented here as it is enough for our purpose, but the full implementation of both the naive and destination-based versions of the whole parser can be found in the software heritage archive of the project\footnote{\TODO{add it}} in \mintinline{text}`src/Compact/Pure/SExpr.hs`.
...
In part \textbf{B} of figure~\ref{fig:bench-charts}, we compared the naive implementation of the S-expression parser and the DPS one (see subsection~\ref{ssec:parser-sexpr}) against a 15,000-lines-long file.
...
\paragraph{Relabeling a tree in a breadth-first fashion}\label{par:benchmark-bf-tree-traversal}
