...
\begin{abstract}
Destination-passing style programming introduces destinations, which represents the address of a write-once memory cell. Those destinations can be passed as function parameters, and thus enable the caller of a function to keep control over memory management: the body of the called function will just be responsible of filling that memory cell. This is especially useful in functional programming languages, in which the body of a function is typically responsible for allocation of the result value.
...
In this article, I present an implementation of destinations for Haskell, which relies on so-called compact regions. I demonstrate, in particular, a 15-40\% improvement over memory allocation in a simple parser example, and 0-50\% improvement in run time.
\end{abstract}
...
\item A first implementation of destinations for Haskell relying on so-called compact regions (section~\ref{sec:implementation}), together with a performance evaluation (section~\ref{sec:benchmark}).
\end{itemize}
...
If we cannot pass type literals through the arguments types, let's pass them through the return type! So we add a new polymorphic type \mintinline{haskellc}`InfoPtrPlaceholder# a`. That type has a phantom type parameter but shares the same memory representation as \mintinline{haskellc}`Addr#`. We can then use a type annotation to provide the type-level literal to the primop: \mintinline{haskellc}`reifyInfoPtr# (# #) :: InfoPtrPlaceholder# a` allows the implementation of \mintinline{haskellc}`reifyInfoPtr#` inside the compiler to read the type parameter \mintinline{haskellc}`a` even though it is both phantom and in return position!
